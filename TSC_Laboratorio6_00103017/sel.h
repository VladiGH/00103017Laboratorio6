...
Matrix createLocalK(int element,mesh &m){
    float D,Ae, E = m.getParameter(THERMAL_CONDUCTIVITY), Acte = m.getParameter(HEAT_SOURCE);
    Matrix K,A,B,Bt,At;
    ...
    productRealMatrix((-E*Ae*Acte)/(2*(D*D)),productMatrixMatrix(Bt,productMatrixMatrix(At,productMatrixMatrix(A,B,2,2,3),2,2,3),3,2,3),K);
